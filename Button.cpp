...
bool Button::onChange(unsigned long currentTime)
{
  //read button state. '0' is pushed, '1' is not pushed.
  bool reading = read();
  ...
  if ((currentTime - m_lastDebounceTime) > DEBOUNCE_DELAY) {
...
bool Button::onPress(unsigned long currentTime)
{
...
bool Button::onRelease(unsigned long currentTime)
{
  //read button state. '1' is pushed, '0' is not pushed.
  bool reading = read();
  // If the switch changed, due to noise or pressing:
